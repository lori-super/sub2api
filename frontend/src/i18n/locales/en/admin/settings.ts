export default {
    settings: {
      ...
      features: {
        ...
        modelPlaza: {
          title: 'Model Prices',
          description: 'Dynamically show available models, official prices, site prices, and effective multipliers.',
          enabled: 'Enable Model Prices',
          enabledHint: 'When enabled, Model Prices appears in the user sidebar.',
          requireAuth: 'Require sign-in to view',
          requireAuthHint: 'When enabled, only signed-in users can view prices. When disabled, visitors can view the public catalogue at /pricing.',
          displayPricingHint: 'Manage official base prices, display multipliers, per-request tiers, and image prices in the separate Display Pricing workspace.',
          configureDisplayPricing: 'Open Display Pricing',
          priceDescription: 'Pricing notes (Markdown)',
          priceDescriptionHint: 'Rendered at the top of the Model Prices page. Use it for display conventions, exchange rates, promotions, etc.',
        },
        pluginManagement: {
      ...
      mediaBridge: {
        title: 'Media Bridge',
        description: 'Materialize Base64 videos that reach the K3 Chat egress as temporary object URLs with runtime capacity, rollout, and storage controls.',
        version: 'Schema v{version}',
        retry: 'Retry',
        hotApplyTitle: 'Applies live after save; no restart required',
        hotApplyDescription: 'The current node applies policy immediately and peers within about 5 seconds. File/capacity policy is request-scoped; memory and R2 protection stay live. Drain stops new bridge work.',
        zeroValue: '0 (automatic or unlimited)',
        zeroValueHint: '0 means no explicit request/byte cap and no bandwidth throttle. Burst 0 equals one second of sustained bandwidth. Memory and R2 protection still apply.',
        modes: {
          off: { label: 'Off', description: 'Do not detect, upload, or rewrite requests.' },
          observe: { label: 'Observe', description: 'Bypass uploads and rewrites while validating the configuration path; match metrics arrive with the monitoring phase.' },
          canary: { label: 'Canary', description: 'Bridge only a stable sample of matching requests.' },
          on: { label: 'On', description: 'Bridge every request allowed by scope and file policy.' },
          drain: { label: 'Drain', description: 'Stop admitting new bridge requests while allowing admitted work to finish.' },
        },
        runtime: {
          title: 'Runtime mode',
          description: 'Control activation, observation, canary rollout, and graceful draining.',
          mode: 'Mode',
          canaryPercent: 'Canary percentage',
          canaryPercentHint: 'Used only in canary mode. Enter 1–100; stable sampling keeps the same request in the same cohort.',
          drainWarning: 'Drain mode does not interrupt in-flight requests, but no new matching request enters the bridge. Wait for capacity to reach zero before maintenance or another transition.',
        },
        scope: {
          title: 'Scope',
          description: 'Media Bridge supports Chat and Responses ingress. This setting can further restrict which request origins trigger bridging; video egress stays fixed to K3 Chat Completions.',
          ingressProtocols: 'Bridge ingress scope',
          ingress: {
            openai_chat_completions: 'Chat Completions',
            openai_responses: 'Responses',
          },
          ingressHint: 'Select any combination. Selecting none means no ingress restriction. This does not disable an API or imply that its official protocol defines a video input field.',
          finalEgress: 'K3 video final egress',
          finalEgressChat: 'Chat Completions (fixed)',
          finalEgressHint: 'Responses requests with explicit video compatibility fields are adapted to Chat before bridging at the fixed Chat egress.',
          vendorExtensionNotice: 'Chat "video_url" and explicit Responses video fields are compatibility extensions, not official OpenAI video semantics. Ordinary image/file fields are never guessed to be video.',
          models: 'Models',
          modelsPlaceholder: 'kimi-k3',
          accountIds: 'Account IDs',
          accountIdsHint: 'Separate positive integers with commas, spaces, or new lines. Leave empty for all accounts.',
          listHint: 'Separate values with commas, spaces, or new lines. Leave empty for no restriction.',
        },
        capacity: {
          title: 'Capacity & tenant fairness',
          description: 'Control capacity primarily by in-flight bytes and bandwidth instead of a fixed concurrency count alone.',
          maxInflightRequests: 'Maximum in-flight requests per node',
          maxInflightRequestsHint: '0 adds no request-count hard cap; memory, in-flight bytes, and R2 health still protect the node.',
          maxInflightBytes: 'Maximum in-flight decoded bytes',
          maxBandwidth: 'Maximum upload bandwidth (Bytes/s)',
          burstBytes: 'Bandwidth burst allowance (Bytes)',
          admissionWaitMs: 'Admission wait limit (ms)',
          admissionWaitMsHint: 'Maximum wait when capacity is unavailable; 0 means no waiting.',
          defaultTenantWeight: 'Default tenant weight',
          defaultTenantWeightHint: 'Must be greater than 0. A higher weight receives a larger fair-scheduling share.',
          tenantOverrides: 'Tenant overrides',
          tenantOverridesHint: 'Add only tenants that differ from global capacity or weight. A zero limit keeps the automatic/unlimited behavior.',
          addTenant: 'Add tenant',
          noTenantOverrides: 'No tenant overrides; every tenant uses the global policy.',
          tenantId: 'Tenant ID',
          weight: 'Weight',
        },
        protection: {
          title: 'Resource & R2 protection',
          description: 'Pause new bridge work and use half-open probes when memory or R2 PUT health crosses a threshold. PUT duration includes intentional throttling, so its latency threshold is off by default.',
          zeroValueHint: 'Use 0 to disable threshold fields. R2 upload timeout must be 1–3600 seconds.',
          memorySoftPercent: 'Memory soft limit (%)',
          memoryHardPercent: 'Memory hard limit (%)',
          minFreeMemory: 'Minimum free memory (Bytes)',
          r2ErrorRate: 'R2 error-rate threshold (%)',
          r2LatencyMs: 'R2 PUT total-duration threshold (ms)',
          r2WindowSeconds: 'R2 measurement window (seconds)',
          r2OpenSeconds: 'Protection open duration (seconds)',
          r2HalfOpenProbes: 'Half-open probes',
          r2MinimumSamples: 'Minimum circuit samples',
          r2UploadTimeoutSeconds: 'R2 upload timeout (seconds)',
        },
        filePolicy: {
          title: 'File policy',
          description: 'Limit bridgeable MIME types, file count, and decoded sizes.',
          allowedMimeTypes: 'Allowed MIME types',
          maxFiles: 'Maximum files per request',
          maxFilesHint: '0 adds no extra limit.',
          maxSingleBytes: 'Maximum decoded bytes per file',
          maxRequestBytes: 'Maximum decoded bytes per request',
          deduplicate: 'Deduplicate within request',
          deduplicateHint: 'Upload identical Base64 content once and reuse the same temporary URL.',
        },
        retention: {
          title: 'URL & object lifecycle',
          description: 'Control upstream read time and delayed deletion after request end. R2 lifecycle rules cover abnormal exits.',
          signedUrlTTL: 'Signed URL TTL (seconds)',
          signedUrlTTLHint: 'Cover upstream queueing, download time, and request retries.',
          requestEndDeleteDelay: 'Delete delay after request end (seconds)',
          requestEndDeleteDelayHint: 'After success, failure, or cancellation, keep a short buffer before deleting the object.',
        },
        storage: {
          title: 'Object storage',
          description: 'Configure dedicated Cloudflare R2 storage for Media Bridge. Saved changes apply only to new requests.',
          provider: 'Provider',
          ...
          secretPlaceholder: 'Enter Secret Access Key',
          secretConfiguredPlaceholder: 'Configured; leave blank to preserve',
          ...
          testing: 'Testing…',
          testSucceeded: 'R2 connection test succeeded',
          testFailed: 'R2 connection test failed',
        },
        validation: {
          canaryPercent: 'Canary percentage must be between 1 and 100 in canary mode.',
          percent: 'Percentage fields must be between 0 and 100.',
          ingressProtocols: 'Bridge ingress scope contains an unsupported protocol. Select it again.',
          accountIds: 'Account IDs must be positive integers separated by commas, spaces, or new lines.',
          nonNegativeInteger: 'Capacity, file, and time fields must be non-negative integers.',
          safetyBounds: 'Time values exceed safe bounds: admission ≤60s, R2 upload 1–3600s, R2 windows ≤24h, and signing/deletion delays ≤7 days.',
          tenantWeight: 'Default tenant weight must be greater than 0.',
          memoryLimits: 'The memory soft limit cannot exceed the hard limit.',
          tenantOverride: 'Tenant ID and weight must be positive integers.',
          duplicateTenant: 'Tenant overrides contain a duplicate tenant ID.',
          mimeTypes: 'The first release supports only video/mp4.',
          retentionTTL: 'Signed URL TTL must be greater than 0.',
          storageProvider: 'The first media-bridge release supports Cloudflare R2 only.',
          storageEndpoint: 'Storage endpoint must be an HTTPS URL without credentials, query parameters, or a fragment.',
          storageBucket: 'Bucket cannot contain whitespace, slashes, or backslashes.',
          objectPrefix: 'Object prefix cannot contain empty path segments, . or ..',
          storageFields: 'Region, Access Key, or Secret contains unsafe characters or exceeds its length limit.',
          storageRequired: 'R2 endpoint and bucket are required before enabling canary or on mode.',
          storageCredentialsRequired: 'Enter the R2 endpoint, bucket, Access Key, and configure a Secret.',
          storageNotReady: 'Pass the R2 connection test before enabling canary or on mode.',
        },
        saved: 'Media bridge settings applied live',
        loadFailed: 'Failed to load media bridge settings',
        saveFailed: 'Failed to save media bridge settings',
      },
      rectifier: {

export default {
    settings: {
      ...
      features: {
        ...
        modelPlaza: {
          title: '模型价格',
          description: '动态展示当前可用模型、官方价格、本站售价与生效倍率。',
          enabled: '启用模型价格',
          enabledHint: '开启后用户侧边栏显示模型价格入口。',
          requireAuth: '需要登录后查看',
          requireAuthHint: '开启时只有登录用户可访问；关闭时游客可通过 /pricing 直接查看公开模型价格。',
          displayPricingHint: '模型官方基础价、展示倍率、按次与生图价格在独立的“展示定价”中管理。',
          configureDisplayPricing: '前往展示定价',
          priceDescription: '价格说明（Markdown）',
          priceDescriptionHint: '展示在模型价格页面顶部，可用于说明展示口径、汇率、优惠活动等。',
        },
        pluginManagement: {
          title: '插件管理',
      ...
      mediaBridge: {
        title: '媒体桥接',
        description: '将最终进入 K3 Chat 出口的 Base64 视频物化为临时对象 URL，并对容量、灰度和存储进行运行时治理。',
        version: 'Schema v{version}',
        retry: '重试',
        hotApplyTitle: '保存后热生效，无需重启',
        hotApplyDescription: '当前节点立即、其他节点约 5 秒读取新策略；文件与容量按请求快照，内存/R2 保护实时生效。排空模式停止新的桥接。',
        zeroValue: '0（自动或不限）',
        zeroValueHint: '请求数/在途字节为 0 表示无显式上限，带宽为 0 表示不限速；Burst 为 0 时等于 1 秒持续带宽。内存与 R2 保护仍生效。',
        modes: {
          off: { label: '关闭', description: '不检测、不上传，也不改写请求。' },
          observe: { label: '观察', description: '旁路验证配置链路，不上传或改写媒体；命中指标将在后续监控阶段接入。' },
          canary: { label: '灰度', description: '只对稳定采样命中的一部分请求执行桥接。' },
          on: { label: '全量开启', description: '对所有符合生效范围和文件策略的请求执行桥接。' },
          drain: { label: '排空', description: '停止接收新的桥接请求，让已入场请求自然完成。' },
        },
        runtime: {
          title: '运行模式',
          description: '控制媒体桥接的启停、观测、灰度和无损排空。',
          mode: '模式',
          canaryPercent: '灰度比例（%）',
          canaryPercentHint: '仅灰度模式使用，范围 1–100；稳定采样避免同一请求反复跳组。',
          drainWarning: '排空模式不会中断在途请求，但新的匹配请求不会进入桥接。确认容量归零后再切换或维护服务。',
        },
        scope: {
          title: '生效范围',
          description: '媒体桥接只接入 Chat 与 Responses；此处可进一步限制哪些请求入口触发桥接，视频出口固定为 K3 Chat Completions。',
          ingressProtocols: '桥接入口范围',
          ingress: {
            openai_chat_completions: 'Chat Completions',
            openai_responses: 'Responses',
          },
          ingressHint: '可多选；全部不选表示不限制入口。此设置不会关闭 API，也不代表对应官方协议定义了视频输入字段。',
          finalEgress: 'K3 视频最终出口',
          finalEgressChat: 'Chat Completions（固定）',
          finalEgressHint: 'Responses 请求若含明确视频兼容字段，会先适配为 Chat，再在固定 Chat 出口桥接。',
          vendorExtensionNotice: 'Chat“video_url”和 Responses 明确视频字段均为兼容扩展，不是 OpenAI 官方视频语义；系统不会把普通 image/file 字段猜测成视频。',
          models: '模型',
          modelsPlaceholder: 'kimi-k3',
          accountIds: '账号 ID',
          accountIdsHint: '用逗号、空格或换行分隔正整数；留空表示所有账号。',
          listHint: '用逗号、空格或换行分隔；留空表示不限制。',
        },
        capacity: {
          title: '容量与租户公平性',
          description: '以在途字节和带宽为主控制容量，避免只依赖固定并发数。',
          maxInflightRequests: '单节点最大在途请求数',
          maxInflightRequestsHint: '0 表示不设请求数硬上限，仍由内存、在途字节和 R2 健康保护。',
          maxInflightBytes: '最大在途解码字节数',
          maxBandwidth: '最大上传带宽（Bytes/s）',
          burstBytes: '带宽突发额度（Bytes）',
          admissionWaitMs: '入场等待上限（ms）',
          admissionWaitMsHint: '容量不足时允许等待的最长时间；0 表示不等待。',
          defaultTenantWeight: '默认租户权重',
          defaultTenantWeightHint: '必须大于 0；权重越高，公平调度份额越大。',
          tenantOverrides: '租户覆盖',
          tenantOverridesHint: '只填写需要偏离全局容量或权重的租户。各限制为 0 时继承自动/不限语义。',
          addTenant: '添加租户',
          noTenantOverrides: '暂无租户覆盖，所有租户使用全局策略。',
          tenantId: '租户 ID',
          weight: '权重',
        },
        protection: {
          title: '资源与 R2 保护',
          description: '内存或 R2 PUT 异常达到阈值时暂停新桥接并半开探测；PUT 延迟包含主动限速时间，默认关闭该延迟阈值。',
          zeroValueHint: '阈值类字段填写 0 表示禁用；R2 上传超时必须为 1–3600 秒。',
          memorySoftPercent: '内存软阈值（%）',
          memoryHardPercent: '内存硬阈值（%）',
          minFreeMemory: '最小可用内存（Bytes）',
          r2ErrorRate: 'R2 错误率阈值（%）',
          r2LatencyMs: 'R2 PUT 总耗时阈值（ms）',
          r2WindowSeconds: 'R2 统计窗口（秒）',
          r2OpenSeconds: '保护打开时长（秒）',
          r2HalfOpenProbes: '半开探测数',
          r2MinimumSamples: '熔断最小样本数',
          r2UploadTimeoutSeconds: 'R2 上传超时（秒）',
        },
        filePolicy: {
          title: '文件策略',
          description: '限制可桥接的 MIME、文件数量和解码后大小。',
          allowedMimeTypes: '允许的 MIME 类型',
          maxFiles: '单请求最大文件数',
          maxFilesHint: '0 表示不额外限制。',
          maxSingleBytes: '单文件最大解码字节数',
          maxRequestBytes: '单请求最大解码字节数',
          deduplicate: '请求内去重',
          deduplicateHint: '相同 Base64 内容只上传一次，并复用同一个临时 URL。',
        },
        retention: {
          title: 'URL 与对象生命周期',
          description: '控制上游可读取时长和请求结束后的延迟删除；异常退出由 R2 生命周期兜底。',
          signedUrlTTL: '签名 URL 有效期（秒）',
          signedUrlTTLHint: '应覆盖上游排队、下载和请求重试时间。',
          requestEndDeleteDelay: '请求结束后延迟删除（秒）',
          requestEndDeleteDelayHint: '无论成功、失败或取消，请求结束后保留一段缓冲时间再删除对象。',
        },
        storage: {
          title: '对象存储',
          description: '配置媒体桥接专用的 Cloudflare R2；保存后只影响新请求。',
          provider: '提供商',
          ...
          secretPlaceholder: '输入 Secret Access Key',
          ...
          test: '测试连接',
          testing: '测试中…',
          testSucceeded: 'R2 连接测试成功',
          testFailed: 'R2 连接测试失败',
        },
        validation: {
          canaryPercent: '灰度模式的比例必须为 1–100。',
          percent: '百分比字段必须为 0–100。',
          ingressProtocols: '桥接入口范围包含不支持的协议，请重新选择。',
          accountIds: '账号 ID 必须是用逗号、空格或换行分隔的正整数。',
          nonNegativeInteger: '容量、文件和时间字段必须是非负整数。',
          safetyBounds: '时间参数超出安全范围：准入 ≤60 秒、R2 上传 1–3600 秒、R2 窗口 ≤24 小时、签名与删除延迟 ≤7 天。',
          tenantWeight: '默认租户权重必须大于 0。',
          memoryLimits: '内存软阈值不能高于硬阈值。',
          tenantOverride: '租户 ID 和权重必须是正整数。',
          duplicateTenant: '租户覆盖中存在重复的租户 ID。',
          mimeTypes: '首期仅支持 video/mp4。',
          retentionTTL: '签名 URL 有效期必须大于 0。',
          storageProvider: '首期媒体桥接仅支持 Cloudflare R2。',
          storageEndpoint: '存储 Endpoint 必须是不含凭据、查询参数或片段的 HTTPS 地址。',
          storageBucket: 'Bucket 不能包含空白、斜杠或反斜杠。',
          objectPrefix: '对象前缀不能包含空路径段、. 或 ..。',
          storageFields: 'Region、Access Key 或 Secret 含非法字符或长度超限。',
          storageRequired: '灰度或全量开启前必须填写 R2 Endpoint 与 Bucket。',
          storageCredentialsRequired: '请完整填写 R2 Endpoint、Bucket、Access Key，并配置 Secret。',
          storageNotReady: '灰度或全量开启前必须先通过 R2 连接测试。',
        },
        saved: '媒体桥接设置已热更新',
        loadFailed: '加载媒体桥接设置失败',
        saveFailed: '保存媒体桥接设置失败',
      },
      rectifier: {
        title: '请求整流器',

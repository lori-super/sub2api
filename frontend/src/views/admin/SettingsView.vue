    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <!-- OpenAI Transport Error Cooldown Settings -->
          <div class="card">
            <div
              class="border-b border-gray-100 px-6 py-4 dark:border-dark-700"
            >
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ t("admin.settings.openaiTransportErrorCooldown.title") }}
              </h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{
                  t("admin.settings.openaiTransportErrorCooldown.description")
                }}
              </p>
            </div>
            <div class="space-y-5 p-6">
              <div
                v-if="openaiTransportErrorCooldownLoading"
                class="flex items-center gap-2 text-gray-500"
              >
                <div
                  class="h-4 w-4 animate-spin rounded-full border-b-2 border-primary-600"
              ...
              <template v-else>
                <div class="flex items-center justify-between">
                  <div>
                    <label class="font-medium text-gray-900 dark:text-white">{{
                      t("admin.settings.openaiTransportErrorCooldown.enabled")
                    }}</label>
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                      {{
                        t(
                          "admin.settings.openaiTransportErrorCooldown.enabledHint",
                        )
                      }}
                    </p>
                  </div>
                  <Toggle v-model="openaiTransportErrorCooldownForm.enabled" />
                </div>

                <div
                  v-if="openaiTransportErrorCooldownForm.enabled"
                ...
                >
                  <div>
                    ...
                    >
                      {{
                        t(
                          "admin.settings.openaiTransportErrorCooldown.cooldownSeconds",
                        )
                      }}
                    </label>
                    <input
                      v-model.number="openaiTransportErrorCooldownForm.cooldown_seconds"
                      type="number"
                      min="1"
                      max="7200"
                      class="input w-32"
                    ...
                    <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                      {{
                        t(
                          "admin.settings.openaiTransportErrorCooldown.cooldownSecondsHint",
                        )
                      }}
                    </p>
                  </div>
                ...
                >
                  {{
                    t(
                      "admin.settings.openaiTransportErrorCooldown.activeCooldownHint",
                    )
                  }}
                </p>
                ...
                >
                  <button
                    type="button"
                    @click="saveOpenAITransportErrorCooldownSettings"
                    :disabled="openaiTransportErrorCooldownSaving"
                    class="btn btn-primary btn-sm"
                  >
                    <svg
                      v-if="openaiTransportErrorCooldownSaving"
                      class="mr-1 h-4 w-4 animate-spin"
                      fill="none"
                      viewBox="0 0 24 24"
                    >
                    ...
                    {{
                      openaiTransportErrorCooldownSaving
                        ? t("common.saving")
                        : t("common.save")
                    }}
                  </button>
          ...
          <div class="space-y-5 p-6">
            ...
            <div v-if="form.model_plaza_enabled" class="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 dark:border-dark-700 dark:bg-dark-900/40">
              <p class="text-sm font-medium text-gray-700 dark:text-gray-200">
                {{ t('admin.settings.features.modelPlaza.displayPricingHint') }}
              </p>
              <router-link to="/admin/channels/display-pricing" class="mt-2 inline-flex items-center gap-1 text-sm font-semibold text-primary-600 hover:text-primary-700 dark:text-primary-300">
                {{ t('admin.settings.features.modelPlaza.configureDisplayPricing') }}
                <span aria-hidden="true">→</span>
              </router-link>
            </div>
          </div>
...
// OpenAI persistent transport failure cooldown state
const openaiTransportErrorCooldownLoading = ref(true);
const openaiTransportErrorCooldownSaving = ref(false);
const openaiTransportErrorCooldownForm = reactive({
  enabled: true,
...
// OpenAI persistent transport failure cooldown methods
async function loadOpenAITransportErrorCooldownSettings() {
  openaiTransportErrorCooldownLoading.value = true;
  try {
    const settings =
      await adminAPI.settings.getOpenAITransportErrorCooldownSettings();
    Object.assign(openaiTransportErrorCooldownForm, settings);
  } catch (_error: unknown) {
    // Silent fail - settings will use defaults
  } finally {
    openaiTransportErrorCooldownLoading.value = false;
  }
}

async function saveOpenAITransportErrorCooldownSettings() {
  openaiTransportErrorCooldownSaving.value = true;
  try {
    const updated =
      await adminAPI.settings.updateOpenAITransportErrorCooldownSettings({
        enabled: openaiTransportErrorCooldownForm.enabled,
        cooldown_seconds:
          openaiTransportErrorCooldownForm.cooldown_seconds,
      });
    Object.assign(openaiTransportErrorCooldownForm, updated);
    appStore.showSuccess(
      t("admin.settings.openaiTransportErrorCooldown.saved"),
    );
  } catch (error: unknown) {
    appStore.showError(
      extractApiErrorMessage(
        error,
        t("admin.settings.openaiTransportErrorCooldown.saveFailed"),
      ),
    );
  } finally {
    openaiTransportErrorCooldownSaving.value = false;
  }
}
...
onMounted(() => {
  ...
  loadOpenAITransportErrorCooldownSettings();
